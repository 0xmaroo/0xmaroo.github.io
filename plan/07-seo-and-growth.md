...
### حالة التنفيذ (2026-09)

**§7.1 — كله متنفّذ ومتحقق منه على المخرجات المبنية:** hreflang على كل صفحة ليها نسختين ·
canonical مطلق · `unlisted` → `noindex,follow` · `<html lang dir>` صح في اللغتين (وده اللي بيخلي
Pagefind يفصل الفهارس — `en: 4` و`ar: 3`) · **sitemap واحد بـ `xhtml:link` بين النسختين**
(22 من 23 رابط؛ الوحيد من غيرها مقال مالوش نسخة عربية، والـ alternate بيتحذف بدل ما يشاور على
صفحة مش موجودة) · **صورة OG لكل مقال ومشروع** بتتولد وقت البناء (العنوان + `CASE 0xx` + الـ CWE).

**§7.2 — متنفّذ، ما عدا بند واحد مقصود:**
`Person` (الهوم + About) · `TechArticle` (كل writeup) · `BreadcrumbList` (الصفحات الداخلية) ·
`WebSite` + `potentialAction: SearchAction` بيشاور على صفحة بحث بتقرا `?q=` فعلاً.

- `sameAs` **رابطين بس** (GitHub + LinkedIn) مش الخمسة المذكورين فوق — الباقي (X، TryHackMe، HTB)
  ما اتأكدش، و§8.2 بتمنع أي رابط غير متحقق.
- `knowsAbout` **مشتق من محتوى الموقع نفسه** (الـ CWE بتاعة المقالات المنشورة + مجالات الـ arsenal
  بلغة الصفحة)، مش مكتوب بالإيد — فمستحيل يبالغ، وبيكبر مع المحتوى. النسخة العربية دلوقتي مفيهاش
  أي CWE لأن مفيش مقال عربي منشور، وده الادعاء الصح.
- **`alumniOf` مش متنفّذ عن قصد** — ده سيرة ذاتية للمالك، مش حاجة تتشتق من الموقع.

**§7.4–§7.7 مش شغل كود** — دي توزيع ومحتوى وقرارات المالك.
